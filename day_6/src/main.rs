...
fn parse_input(input: String) -> Vec<Point> {
    ...
    for line in input.lines() {
        ...
        ret.push(Point {x: cap[1].parse().unwrap(), y: cap[2].parse().unwrap()});
    }
...
//                println!("{:?}", distance);
            ...
            print!("{}", (closest_i + 97) as u8 as char);
    ...
    let banned_c: std::collections::HashSet<char> = banned.iter().map(|x| (x + 97) as u8 as char).collect();
    println!("{:?}", banned_c);
    for (i, c) in counts.iter().enumerate() {
        println!("{}: {}", (i + 97) as u8 as char, c);
    }

    *counts.iter().enumerate().filter(|x| !banned.contains(&x.0)).map(|x| x.1).max().unwrap()
}
